...
class Attraction(BaseModel):
    ...
    visit_duration: int = Field(..., description="建议浏览时间（分钟）", ge=0)
    description: str = Field(..., description="景点描述")
    category: str | None = Field(default="景点", description="景点类别")
    rating: float | None = Field(default=None, description="景点评分", ge=0, le=5)
...
class TripPlan(BaseModel):
    ...
    weather_info: list[WeatherInfo] = Field(
        default_factory=list, description="每日天气"
    )
    overall_suggestions: str = Field(..., description="总体建议")
    budge: Budget | None = Field(default=None, description="预算信息")
